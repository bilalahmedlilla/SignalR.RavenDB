...
namespace SignalR.RavenDB
{
    public class RavenMessageBus : ScaleoutMessageBus
    {
                ...
                using (var session = _documentStore.OpenAsyncSession())
                {
                    try
                    {
                        ...
                        if (_expiration > TimeSpan.Zero)
                        {
                            ...
                            var metadata = session.Advanced.GetMetadataFor(ravenMessage)[RavenExpirationDate] = expiry.ToString("o"); // Store expiry as ISO 8601 string
        ...
        private void Subscribe(IDatabaseChanges changes)
        {
            ...
            _subscription = changes.ForAllDocuments().Subscribe(_observer);
